...
class Map( Entity ):
    ...
    def __init__( self, group=None, **kwargs ):
        Entity.__init__( self, (80, 80), [0,0], None, group, pygame.Rect( 0, 0, self.width, self.height ), animated=False, **kwargs )
        if Map.instanceSpecificVars is None:
            attrList = list( self.__dict__.keys() )
        if Map.instanceSpecificVars is None:
            Map.instanceSpecificVars = dict( [ ( eachKey, eachVal ) for eachKey, eachVal in self.__dict__.items() if eachKey not in attrList ] )
